...
'| Descrição: Arquivo de Formulário para cadastro e Cliente (Javascript)
'|--------------------------------------------------------------------
*/
...
//				document.getElementById("btnBuscarCepColeta").style.cursor = 'pointer';
	...
	function validaCadClienteContato() {
  		...
  		if (!is_email(form.txtEmail.value))
		{
  		...
  		if (form.txtDDD.value == "") {
  		    alert("Preencha o campo DDD do usuário Master");
  		    return false;
  		}
  		if (form.txtDepartamento.value == "") {
  		    alert("Preencha o campo Departamento do usuário");
  		    return false;
  		}
  		if (isNaN(form.txtDDD.value)) {
  		    alert("Preencha o campo DDD do usuário Master somente com Números!");
  		    return false;
  		}
  		if (form.txtDDD.value.length != 2) {
  		    alert("Preencha o campo DDD com 2 dígitos!");
  		    return false;
  		}
  		if (form.txtTelefone.value == "") {
  		    alert("Preencha o campo Telefone do usuário");
  		    return false;
  		}
  		if ( isNaN(form.txtTelefone.value)) {
  		    alert("Preencha o campo Telefone do usuário somente com números!");
  		    return false;
  		}
  		if (form.txtTelefone.value.length < 8) {
  		    alert("Preencha o campo Telefone com no mínimo 8 dígitos!");
  		    return false;
  		}
  		if (form.txtRamal.value == "") {
  		    alert("Preencha o campo Ramal do usuário Master");
  		    return false;
  		}
  		if (isNaN(form.txtRamal.value)) {
  		    alert("Preencha o campo Ramal somente com números!");
  		    return false;
  		}

  		alert("Cadastro efetuado! Por gentileza aguardar a aprovação do mesmo. Você receberá um e-mail de confirmação do cadastro e logo entraremos em contato!");
  		return true;
	...
	function checkCNPJEmpresa() {
	    ...
	    if (!nret) {
	        alert("CNPJ com dígito verificador inválido! Corrija o número do CNPJ!");
	...
	function checkCPF() {
	    ...
	    if (!nret) {
	        alert("CPF com digito verificador inválido! Corrija o número do CPF.");
	        document.frmCadCliente.txtCPFnum.focus();
	        return false;
	    }
		...
		oAjax.open("GET", "ajax/frmCadCliente.asp?sub=getcheckcpf&id="+document.frmCadCliente.txtCPFnum.value, true);
		oAjax.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=iso-8859-1");
		oAjax.send(null);
	}
	...
	function validateCPF() {
		...
		if (form.txtNCPF.value.length < 14) {
			msgErrForm += "Campo: CPF\n";
			errForm = true;
			return false;
		}
	...
	function onblurie(value) {
	    var txt = value.split(".").join("");
	    if (isNaN(txt)) {
	        if (txt.toUpperCase() == "ISENTO") {
	            return true;
	        }
	        if (txt.toUpperCase() != "ISENTO") {
	            alert("IE inválida! Para IE diferente de números, só é permitido: ISENTO");
	            document.frmCadCliente.txtIE.focus();
	            return false;
	        }
	    }

		return true;//retirada a validação de números
		
        //caso seja número
	    if (!isNaN(txt)) {
	        if (txt.length!=12) {
	            alert("IE inválida! Formato válido: 999.999.999.999");
	            document.frmCadCliente.txtIE.focus();
	            return false;
	        }
	    }
	...
	function keypressIE(value) {
	    var form = document.frmCadCliente;
	    var txt = value.split(".").join("");
		return;//alterado conforme solicitação da Elaine no dia 11-7-2014; 
        if (!isNaN(txt)) {
	...
	cnpj = document.frmCadCliente.txtCPFnum.value;

	digVer2 = cnpj.charAt(cnpj.length - 1);
	digVer1 = cnpj.charAt(cnpj.length - 2);
	
	if (document.frmCadCliente.txtCPFnum.value == "") {
		//alert("Preencha o campo CPF!");
		return false;
	}
	if (document.frmCadCliente.txtCPFnum.value.length < 14) {
