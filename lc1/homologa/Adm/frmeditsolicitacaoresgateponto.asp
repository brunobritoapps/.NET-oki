...
'		Response.End()
		...
		sSql = "SELECT [idSolicitacoes_resgate] " & _
				  ...
				  ",[quantidade] " & _
				  ",[idcliente] " & _
			  "FROM [marketingoki2].[dbo].[Solicitacoes_resgate_Ponto] WHERE [idsolicitacao] = " & Request.QueryString("idsolic")
...
'		response.end
	...
	Sub GetCliente()
		...
		sSql = "SELECT " & _
						...
						"ON A.[idcliente] = B.[idpontos_coleta] " & _
	...
	function getMoeda(id)
		...
		sql = "select a.bonus_type from pontos_coleta as a " & _
				"left join cadastro_bonus as b " & _
				"on a.bonus_type = b.cod_bonus " & _
				"where a.idpontos_coleta = " & id
	...
	Sub UpdateSol()
		...
		select case request.Form("btnAprovar")
			case "Aprovar"
				...
				response.write sSql & "<br />"
				Call exec(sSql)
				sSql = "UPDATE [marketingoki2].[dbo].[Solicitacoes_resgate_Ponto] " & _
...
<script>
	function validaFormResgate() {
		...
		//alert(form.txtDataAprovacao.value);
		//if (form.cbStatusSolColeta.value != 2) {
			<%If StatusSol <> 1 Then%>
...
<div id="container">
	<div id="conteudo" style="height:100%;">
		<form action="frmeditsolicitacaoresgateponto.asp" name="frmEditSolicitacaoColetaDomiciliarAdm" method="POST">
		...
		<table cellpadding="1" cellspacing="1" width="500" align="left" id="tableEditSolicitacaoColetaAdm" border="0">
			...
			<tr id="trnumsolcoleta">
				...
				<td><%=NumSolColeta%>&nbsp;<img src="img/buscar.gif" class="imgexpandeinfo" align="absmiddle" alt="Buscar Solicitações que compuseram a solicitação Master" onClick="javascript:window.open('frmviewsolicitacaocompoeresgateponto.asp?idsolic=<%=NumSolColeta%>','','width=650,height=250,scrollbars=no,status=no,location=no,toolbar=no,menubar=no');"/></td>
			</tr>
			<tr id="tridcliente">
			...
			<tr id="trdatarecebimento">
				...
				<td>
				<input type="text" <%If isNull(DataBaixa) Or DataBaixa = "" Then%>class="textreadonly"<%ELse%>class="text"<%End If%> name="txtDataBaixa" value="<%=DataBaixa%>" size="13" maxlength="10" onKeyPress="date(this)" readonly />
