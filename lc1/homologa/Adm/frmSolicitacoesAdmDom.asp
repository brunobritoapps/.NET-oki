	...
	Sub getSolicitacoesColeta()
        ...
        if trim(request.querystring("busca")) <> "" then
			sSql = "SELECT " & _ 
							...
							"[A].[qtd_cartuchos], " & _ 
							"convert(date, A.data_recebimento)," & _
							"[A].[motivo_status], " & _ 
							"[A].[isMaster], " & _
							"[C].[nome_fantasia], " & _
							"[C].[typeColect], " & _
							...
							"[d].[status_coleta], " & _
                            "convert(date, A.data_solicitacao) " & _
                            ...
                            if trim(request.querystring("busca0")) <> "" then
                                sSql = sSql & " and [C].[cnpj] = '" & trim(request.querystring("busca0")) & "'"
                            End if

                            if trim(request.querystring("busca1")) <> "" then
                                sSql = sSql & " and [C].[razao_social] like '" & trim(request.querystring("busca1")) & "'"
                            End if

        Elseif trim(request.querystring("busca0")) <> "" then
			sSql = "SELECT " & _ 
							"[A].[idSolicitacao_coleta], " & _
							"[A].[numero_solicitacao_coleta], " & _ 
							"[A].[qtd_cartuchos], " & _ 
							...
							"on [A].[status_coleta_idstatus_coleta] = [d].[idstatus_coleta] " & _
							"WHERE [B].[typeColect] = 1 AND " & _
							"[C].[cnpj] = '" & trim(request.querystring("busca0")) & "'"

                            if trim(request.querystring("busca1")) <> "" then
                                sSql = sSql & " and [C].[razao_social] like '" & trim(request.querystring("busca1")) & "'"
                            End if

        Elseif trim(request.querystring("busca1")) <> "" then
    			sSql = "SELECT " & _ 
							"[A].[idSolicitacao_coleta], " & _
							"[A].[numero_solicitacao_coleta], " & _ 
							"[A].[qtd_cartuchos], " & _ 
							...
							"on [A].[status_coleta_idstatus_coleta] = [d].[idstatus_coleta] " & _
							"WHERE [B].[typeColect] = 1 AND ( " & _
							"upper([C].[razao_social]) like upper('" & trim(request.querystring("busca1")) & "%') or " & _
                            "upper([C].[nome_fantasia]) like upper('" & trim(request.querystring("busca1")) & "%') " & _
                            " ) "
        ...
        sSql = sSql & " order by [A].[data_solicitacao] desc "
			...
			If intSolColeta > -1 Then
				...
				For i = (intProdsPorPag * (intPag - 1)) to intUltima
					If i Mod 2 = 0 Then						
    				    ...
    				    If IsNull(arrSolColeta(11,i)) Then
							.Write	"<td align='left' width='15%' class='classColorRelPar'>&nbsp;&nbsp;/&nbsp;&nbsp;/&nbsp;&nbsp;&nbsp;&nbsp;</td>"
						Else
							.Write	"<td class='classColorRelPar'>"&DateRight(arrSolColeta(11,i))&"</td>"
						End If
					...
					Else
						...
						Else
							.Write	"<td class='classColorRelImpar'>"&DateRight(arrSolColeta(11,i))&"</td>"
						End If
				...
				<td id="conteudo">
					<table cellpadding="3" cellspacing="0" width="100%">
						...
						<tr>
							...
							<td id="explaintitle" align="left" class="auto-style2">
                                <input type="text" name="busca0" class="text" value="<%= request.querystring("busca0") %>" size="30" maxlength="18"/></td>
							<td id="explaintitle" align="left" class="auto-style3" colspan="2">
								Digitação Completa:<br />
                                Ex CNPJ: 99.999.999/9999-99 |
						...
						<tr>
							<td id="explaintitle" align="left" class="auto-style1" style="text-align:right;">
								...
								Nome Fantasia:</td>
							<td id="explaintitle" align="left" class="auto-style2">
                                <input type="text" name="busca1" class="text" value="<%= request.querystring("busca1") %>" size="40" maxlength="30"/></td>
							<td id="explaintitle" align="left" class="auto-style4" colspan="2">
								Este campo faz busca aproximada pelas iniciais pelo nome fantasia ou razão social.</td>
						</tr>
						<tr>
							<td id="explaintitle" align="left" class="auto-style1" style="text-align:right;">
								&nbsp;</td>
							...
							<td id="explaintitle" align="left">
								<input type="button" name="btnprocurar" value="Procurar" class="btnformMaior" onClick="window.location.href = 'frmSolicitacoesAdmDom.asp?busca=' + document.frmOperacionalAdm.busca.value + '&busca0=' + document.frmOperacionalAdm.busca0.value + '&busca1=' + document.frmOperacionalAdm.busca1.value + ''" /></td>
						</tr>
						<tr>
							<td align="left" class="auto-style5" colspan="4" style="font-size:1px;">
								</td>
						</tr>
						<tr>
							<td colspan="4" id="explaintitle" align="left">
