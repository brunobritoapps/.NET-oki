...
session("sql") = ""
response.redirect("frmRelatorioBonusPontoColeta.asp")
end if
%>
...
<%
	...
	sub exportarParaArquivo(sql)
		...
		if intarr > -1 then
			...
			for i=0 to intarr
				arquivo = arr(4,i)&";"&arr(7,i)&";"&arr(8,i)&";"&arr(3,i)&";"&arr(0,i)&";"&arr(5,i)&";"&DateRight(arr(1,i))&";"&DateRight(arr(2,i))
...
'			response.Redirect "http://localhost:81/sgrs/adm/exportacao/"&filenamecsv
	...
	function getBonusCliente()
		...
		sql = "SELECT " & _
				...
				"ON A.[cod_bonus] = C.[cod_bonus] "
								
				'response.write sql&"<br>"
				if request.servervariables("HTTP_METHOD") = "POST" then
					call setRequest()
					sql = sql & getWhereSQL()
					sql = sql & " GROUP BY A.[cod_bonus] , A.[data_geracao] , A.[data_validade] , A.[numero_solicitacao] , A.[moeda], A.[saldo] , B.[razao_social] , B.[estado] "	
