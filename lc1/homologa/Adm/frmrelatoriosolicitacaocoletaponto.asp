...
<%
	...
	sub exportarParaArquivo(sql)
		...
		if intarr > -1 then
			cabecalhoArq = "Data Solicitação;Número Solicitação;Cod. Cliente;Razão Social;Qtd. Cartuchos;Data de Entrega;Status"
			arquivoPath.writeLine(cabecalhoArq)
			for i=0 to intarr
				arquivo = DateRight(arr(5,i))&";"&arr(2,i)&";"&arr(15,i)&";"&arr(16,i)&";"&arr(3,i)&";"&DateRight(arr(9,i))&";"&GetStatusDesc(arr(1,i))
	...
	function getSolicitacaoByPonto()
		...
		sql = "SELECT " & _
				...
				"LEFT JOIN cep_consulta_has_clientes as d " & _
				"on c.idclientes = d.clientes_idclientes " & _
				"WHERE b.pontos_coleta_idpontos_coleta = " & session("IDPonto")
...
'		response.end
		...
		if intarr > -1 then
			for i=0 to intarr
				...
				if not isnull(arr(9,i)) then
					html = html & "<td "&style&">"&DateRight(formatdatetime(arr(9,i),2))&"</td>"
				else
					html = html & "<td "&style&"></td>"
				end if	
...
'		Response.Write dataEntregaPonto_ate & "<br />"
	...
	function existWherePonto()
		if  cint(Request.Form("status")) <> 0 or _
			...
			len(Trim(Request.Form("dedatacadastro"))) > 0 and len(Trim(Request.Form("atedatacadastro"))) > 0 then
								...
								<tr>
									<td width="80%">
										<fieldset style="font-size:10px;font-family:Verdana, Arial, Helvetica, sans-serif;">
											...
											<div align="left" style="padding:3px 3px 3px 3px;width:100%;">Status:
											...
											<div align="left" style="padding:3px 3px 3px 3px;width:100%;">Data de entrega no Ponto de Coleta -
												De: <input name="dedataentrega" type="text" class="text" value="<%=Trim(Request.Form("dedataentrega"))%>" size="13" /> 
