...
<%
	...
	function getSolicitacoesByCliente()
		dim sql, arr, intarr, i, pSql
		dim html, style
		
        pSql = "SELECT " & _ 
                " convert( nvarchar(10), A.[data_aprovacao], 103) as 'Data de Aprovacao da Coleta' " & _
                ",convert( nvarchar(10), A.[data_solicitacao], 103) as 'Data de Solicitacao do Pedido de Coleta' " & _
                ",A.[numero_solicitacao_coleta] as 'Numero Solicitacao de Coleta' " & _
                ",' ' as 'Usuario Solicitante' " & _
                ",A.[qtd_cartuchos] as 'Qtd.Cartuchos' " & _
                ",D.[quantidade] as 'Qtd.Volumes' " & _
				",Upper(B.logradouro_coleta) as 'Logradouro', convert( nvarchar,B.numero_endereco_coleta,10) as 'Numero', Upper(B.bairro_coleta) as 'Bairro', Upper(B.municipio_coleta) as 'Municipio', B.estado_coleta as 'Estado', B.cep_coleta as 'CEP' " & _ 
                ",CEPCLI.logradouro as 'Logradouro', RTrim(convert(nvarchar,B.numero_endereco_coleta,10)) as 'Numero', Upper(Rtrim(CEPCLI.bairro)) as 'Bairro',  RTrim(CEPCLI.municipio) as 'Municipio', CEPCLI.estado  as 'Estado', CEPCLI.cep as 'CEP'  " & _ 
                ",' ' as 'LR Num.' " & _  
                 ",TRANSP.nome_fantasia as 'Transportadora' " & _
                ",C.[razao_social] as 'Razao Social/Nome' " & _
                ...
                ",D.[Produtos_idProdutos] as 'Codigo Recebido' " & _ 
                ",E.[descricao] as 'Descricao' " & _ 
                ", STATUSCOL.status_coleta as 'Status Solicitacao' " & _ 
				...
				"LEFT JOIN [cep_consulta_has_Clientes] as CEPCLI on CEPCLI.Clientes_idClientes = C.idClientes and  CEPCLI.isEnderecoColeta = 0 " & _
        ...
        sSql = "SELECT " & _ 
                ...
                ",A.[qtd_cartuchos] as 'Qtd.Cartuchos' " & _
                ",D.[quantidade] as 'Qtd.Volumes' " & _
                ",Upper(B.logradouro_coleta) + ',' + convert( nvarchar,B.numero_endereco_coleta,10)+ ' - ' + Upper(B.bairro_coleta) + ' - ' + Upper(B.municipio_coleta) + ' - ' + B.estado_coleta + ' - CEP: ' + B.cep_coleta as 'Dados para Faturamento' " & _ 
                ",CEPCLI.logradouro + ',' + RTrim(convert(nvarchar,B.numero_endereco_coleta,10)) + ' - ' + Upper(Rtrim(CEPCLI.bairro)) + ' - ' + RTrim(CEPCLI.municipio) + ' - ' + CEPCLI.estado + ' - CEP: ' + CEPCLI.cep as 'Dados para Coleta' " & _ 
                ",' ' as 'LR Num.' " & _ 
                ",TRANSP.nome_fantasia as 'Transportadora' " & _
                ",C.[razao_social] as 'Razao Social/Nome' " & _
                ",C.[nome_fantasia] as 'Nome Fantasia'  " & _
		...
		if request.ServerVariables("HTTP_METHOD") = "POST" then
			...
			if existWhereCliente() then
				...
				pSql = pSql & getWhereSQLCliente()'incluido para enviar os dados corretos para o excel. #alt141015
				session("sql") = sql
			end if
			else
			if session("sql") <> "" then
					...
					<tr>
						<td colspan="3">
							<table cellpadding="1" cellspacing="1" width="100%" id="tableRelSolPendente" style="border:1px solid #000000">
                                ...
                                <th width="10%">Dados para Faturamento</th>
                                <th width="10%">Dados para a Coleta	</th>
