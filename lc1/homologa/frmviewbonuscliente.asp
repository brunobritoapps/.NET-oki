...
<%
	...
	function getSolicitacoesByCliente()
		...
		sql = "select distinct(bonus.numero_solicitacao) " & _
			  ...
			  "where bonus.data_validade >= GETDATE() AND bonus.clientes_idclientes in (select idclientes from clientes where cod_cli_consolidador = "&session("IDCliente")&" or idclientes = "&session("IDCliente")&")"
...
'						html = html & "</tr>"
			...
			'html = html & "<td colspan=""6"" align=""right""><a href=""frmviewopcoesresgate.asp"" class=""linkOperacional"">Opções de Resgate</a>&nbsp;&nbsp;&nbsp;&nbsp;<b>Saldo Acumulado:</b> "&saldoTotal&"&nbsp;&nbsp;</td>"
            html = html & "<td colspan=""2"" align=""left""><b>Saldo Acumulado:</b> "&saldoTotal&"&nbsp;&nbsp;</td>"
			html = html & "</tr>"
			getSolicitacoesByCliente = html
		else
			html = html & "<tr><td colspan=""8"" align=""center"">Nenhum bônus foi gerado até o momento</td></tr>"
			getSolicitacoesByCliente = html
		end if
	end function
