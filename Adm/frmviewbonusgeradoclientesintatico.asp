...
'	end function
	...
	function getSolicitacoesByCliente(id)
		...
		sql = "select distinct(bonus.numero_solicitacao) " & _
			  ...
			  "where bonus.clientes_idclientes in (select idclientes from clientes where cod_cli_consolidador = "&id&" or idclientes = "&id&")"
	'		response.write sql
	'		response.end
			call search(sql, arr, intarr)
			if intarr > -1 then
...
'						html = html & "</tr>"
						...
						html = html & "<tr>"
						html = html & "<td "&style&"><img src=""img/buscar.gif"" class=""imgexpandeinfo"" onclick=""window.location.href='frmviewbonusgeradocliente.asp?id="&arr(0,i)&"'"" /></td>"
