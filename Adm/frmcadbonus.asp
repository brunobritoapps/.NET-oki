		...
		if intgrupos > -1 then
			for n=0 to intgrupos
				...
				if intarr > -1 then
					...
					ret = ret & "<th>ID</th>"
					ret = ret & "<th>Descrição</th>"
					ret = ret & "<th>quantidade</th>"
					ret = ret & "<th>pontuação</th>"
					ret = ret & "<th>pontuação target</th>"
					ret = ret & "</tr>"
					for i=0 to intarr
						if i mod 2 = 0 then
							style = "class=""classColorRelPar"""
...
'		response.end
		...
		'aqui q vou mudar a forma de inclusão, tem q buscar da tabela temp
		'sql = "select * from Cadastro_bonus_has_produtos_temp where SessionId = " & Session.SessionID
        sql = "select * from Cadastro_bonus_has_produtos where SessionId = " & Session.SessionID 'peterson aquino 17-5-2014 id:6
		...
		if intarr > -1 then
			...
			sql = "INSERT INTO [marketingoki2].[dbo].[Cadastro_bonus_has_produtos] " & _
						   ...
						   ",[pontuacao_target] " & _
						   ",[cad_cod_bonus]) " & _
                            "select [idoki_prod] ,[qtd] ,[pontuacao] ,[pontuacao_target], '"&trim(cod_bonus)&"' as cad_cod_bonus from Cadastro_bonus_has_produtos where sessionid="&Session.SessionID
					 	   '"select [idoki_prod] ,[qtd] ,[pontuacao] ,[pontuacao_target], '"&trim(cod_bonus)&"' as cad_cod_bonus from Cadastro_bonus_has_produtos_temp where sessionid="&Session.SessionID 'peterson aquino 17-5-2014 id:6
			'Response.Write sql & "<hr>"
			'Response.End
			call exec(sql)
			'call exec("delete from Cadastro_bonus_has_produtos_temp where sessionid="&Session.SessionID)
            'call exec("delete from Cadastro_bonus_has_produtos where sessionid="&Session.SessionID) 'peterson aquino 17-5-2014 id:6
		else
		end if
...
/*
		for (var i=0;i < document.getElementById("totalprodutos").value;i++) {
			...
			if (contObject == parseInt(document.getElementById("totalprodutos").value)) {
				alert("Por favor escolha um Produto para esse Bônus");
				return;
			}
			if (erro) {
				alert(msg);
...
*/		
		...
		if (form.validade.value == "") {
			alert("Por favor preencha o campo Validade( Quantidade em DIAS )");
			return;
		}
		...
		if (form.cbAplicacao.value == -1) {
			alert("Por favor escolha um tipo de Aplicação");
			return;
		}
		if (form.data_inicio.value == "" || form.data_inicio.value == "___/___/___") {
			alert("Por favor preencha o início da Contabilização do Bônus");
			return;
		}
		form.submit();
	}
...
function populateList() {
	if (frmcadbonus.Grupos.options.selectedIndex >= 0 ){
		var Grupo = frmcadbonus.Grupos.options[frmcadbonus.Grupos.options.selectedIndex].value;
		...
		alert("Selecione pelo menos um grupo para realizar a pesquisa!");
	}
      ...
      <form action="frmcadbonus.asp" name="frmcadbonus" method="POST">
        <tr>
          ...
          <td id="conteudo"><table cellspacing="1" cellpadding="1" width="100%" border=0 id="tableEditSolicitacaoColetaAdm">
              <tr>
                <td colspan="5" id="explaintitle" align="center">Cadastro de Bônus</td>
              </tr>
              <tr>
                <td colspan="5" align="right"><a class="linkOperacional" href="javascript:window.location.href='frmOperacionalAdm.asp';">&laquo Voltar</a></td>
              </tr>
              <tr>
                <td width="35%" align="right">Cód. Bônus:</td>
                <td><input type="text" name="cod_bonus" class="textreadonly" value="<%= cod_bonus %>" maxlength="10" readonly="readonly"/>
                    <input type="button" class="btnform" name="listar" value="Listar Bônus" onClick="javascript:window.open('frmlistabonus.asp','','width=700,height=400,scrollbars=no,status=no,location=no,toolbar=no,menubar=no');" /></td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
              </tr>
              <tr>
                <td width="35%" align="right">Descrição:</td>
                <td colspan="3"><textarea name="textdesc" cols="50" rows="10" ><%= desc_bonus %></textarea></td>
              </tr>
              <tr>
                <td width="35%" align="right">Validade em Dias:</td>
                <td colspan="3"><input type="text" name="validade" class="textreadonly" value="<%= validade %>" size="6" />
                  * </td>
              </tr>
              <tr>
                <td width="35%" align="right">Moeda:</td>
                <td colspan="3"><select name="cbMoeda" class="select">
                    <option value="-1">[Selecione]</option>
                    <option value="P" <%if moeda = "P" then%>selected<%end if%>>PONTUAÇÃO</option>
              ...
              <tr>
                <td width="35%" align="right">Aplicação:</td>
                <td colspan="3"><select name="cbAplicacao" class="select">
                    <option value="-1">[Selecione]</option>
              ...
              <tr>
                <td width="35%" align="right">Data início Contabilização:</td>
              ...
              <tr>
                <td colspan="4" align="center"><fieldset style="width:700px;padding:5px 5px 5px 5px;">
                  <legend style="color:#DE8989;font-weight:bold;">Ações</legend>
                  <div align="right">
                    &nbsp;<input type="button" class="btnform" name="cad_submit" value="<%if len(trim(request.querystring("id"))) <> 0 then%>Editar Bônus<%else%>Cadastrar Bônus<%end if%>" onClick="validaForm()" />
                  </div>
                  </fieldset></td>
              </tr>
              <tr>
                  ...
                  <input type="text" name=t1 value="" size=20>
                    ...
                    <table width=100% border=0>
											<tr>
												...
												<td>&nbsp;</td>
												<td>
													Procure pelo Código ou Descrição: <br>
													
										<INPUT type="text" name=txtSearchProd class="textreadonly">
										...
										<select name="Grupos" id="Grupos" onChange="populateList();" class="controls" style="width: 180px; height: 210px; font-size: 13px;" size="8">
												...
												<td>&nbsp;</td>
												<td><INPUT type="button" value="Listar" onClick="MoveSelectedListItems(document.getElementsByName('Prods')[0])"></td>
											</tr>
										</table>
