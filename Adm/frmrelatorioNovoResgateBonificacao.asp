...
session("sql") = ""
response.redirect("frmrelatorioNovoResgateBonificacao.asp")
end if
%>
<%Call open()%>
<%Call GetSessionAdm()%>
<%
	...
	function getBonusCliente()
        ...
        sNovaQry = "" & _ 
                ...
                "	                LEFT JOIN [marketingoki2].[dbo].[Clientes] AS CLI ON CLI.idClientes = SOLRESG.idcliente   " & _ 
                "					LEFT JOIN [dbo].[Solicitacao_Resgate_has_Solicitacao_Composicao] as COMPOSI  ON COMPOSI.numero_resgate = SOLRESG.numero_solicitacao_geracao AND COMPOSI.numero_solicitacao <> '' " & _ 
                "					LEFT JOIN [dbo].[Solicitacao_coleta] as SOLCOL ON SOLCOL.numero_solicitacao_coleta = SOLRESG.numero_solicitacao_geracao " & _ 
                "					LEFT JOIN [dbo].[Bonus_Gerado_Clientes] AS BONUSGER ON BONUSGER.numero_solicitacao = COMPOSI.numero_solicitacao and BONUSGER.idproduto = SOLRESG.idproduto " & _ 
