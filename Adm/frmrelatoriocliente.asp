...
<%
   ...
   sub exportarParaArquivo(sql)
	...
	Sub getClientes()
        ...
        sNovaQry = "SELECT " & _ 
                    "A.[razao_social] as 'Razao Social', " & _ 
                    ...
                    "A.[numero_endereco] as 'Numero', " & _
                    "E.bairro as 'Bairro', " & _
                    "E.municipio as 'Municpio', " & _
                    "E.estado as 'Estado', " & _
                    "' ' as Regiao, " & _
                    "A.[ddd] as DDD, " & _ 
                    "A.[telefone] as Telefone, " & _ 
                    "B.[descricao] as 'Descricao Categoria', " & _
                    "Rtrim(A.[cod_bonus_cli]) as 'Cliente Bonificado?', " & _
                    "d.descricao as 'Descricao Bonus', " & _
                    "convert(nvarchar(10), A.data_atualizacao_sequencial, 103) as 'Data Cadastro', " & _
                    "case " & _ 
			...
			For i = (intProdsPorPag * (intPag - 1)) to intUltima			
                ...
                if arrClientes(18,i) <> "" then
					If i Mod 2 = 0 Then
						...
						Response.Write "<td class='classColorRelPar'>"&arrClientes(07,i)&"</td>"				'endereço
                        ...
                        if len(trim(arrClientes(15,i))) = 0 Then
                            Response.Write "<td class='classColorRelPar'>Não</td>"   'cliente bonificado?
                        else
					...
					Else
						...
						Response.Write "<td class='classColorRelimPar'>"&arrClientes(07,i)&"</td>"				'endereço
                        ...
                        if len(trim(arrClientes(15,i))) = 0 Then
                            Response.Write "<td class='classColorRelimPar'>Não</td>"   'cliente bonificado?
                        else
...
'		response.end
	...
	function existWhere()
		if  len(trim(razaosocial)) > 0 or _	
			...
			'Alteraçào feita por Wellington
			'Programador: Wellington
			'Descrição: Para que a função detecte valores nas datas do cadastro			
			existWhere = true
		else
			existWhere = false
		end if	
	...
	function getWhere()
		...
		if existWhere() then	
            ...
            'Alteraçào feita por Wellington
            'Programador: Wellington
            'Descrição: Para que a cláusula SQL pegue a data inicial e final DE CADASTRO
...
<link rel="stylesheet" type="text/css" href="../css/geral.css">
    ...
    <!-- OVERRIDE[3]Brito-Loop-28-10-18{ INCLUSAO DE BIBLIOTECA JQUEY PARA MANIPULACAO DE ELEMENTOS E EVENTOS HTML }-->
...
function formValidate() {
 ...
 //INCLUDE[4]Brito-Loop-28-10-18{ Funcao para limpar campos de datas }-->
    function limparCampos() {
        $('#status option').eq(0).prop('selected', true);
        $("input[name='razaosocial']").val("");
        $('#grupos option').eq(0).prop('selected', true);
        $('#categoria option').eq(0).prop('selected', true);

        $("input[name='debonus']").val("");
        $("input[name='atebonus']").val("");

        $("input[name='dedatacadastro']").val("");
        $("input[name='atedatacadastro']").val("");
    }

    //INCLUDE[5]Brito-Loop-28-10-18 { Verifica campos preenchidos conforme validacao existWhere() e concatena query correspondetes aos mesmos}
		...
		<tr> 
			<td id="conteudo">
				<table cellspacing="3" cellpadding="2" width="100%" border=0>
					<tr>
						<td colspan="3" id="explaintitle" align="center">Relatório de Clientes</td>
					</tr>
					<tr>
					...
					<tr>
                        ...
                        &nbsp;&nbsp;&nbsp; <br>
                        Razão Social: 
                        <input id="razaosocial" type="text" class="text" name="razaosocial" value="<%= Request.Form("razaosocial") %>" size="170" />
											</div>
											<div align="left" style="padding:3px 3px 3px 3px;width:100%;">
												Grupos:
												<select id="grupos" name="grupos" class="select" style="width:200px;">
													<option value="0">[Selecione]</option>
													<%= getGrupos() %>
												</select>
												&nbsp;&nbsp;&nbsp;
												Cód. Bônus -
												De: <input id="debonus" type="text" class="text" name="debonus" value="<%= Request.Form("debonus") %>" size="13" />
												Até: <input id="atebonus" type="text" class="text" name="atebonus" value="<%= Request.Form("atebonus") %>" size="15"  />
											</div>
											<div align="left" style="padding:3px 3px 3px 3px;width:100%;">
												Categoria:
												<select id="categoria" name="categoria" class="select" style="width:300px;">
													<option value="0">[Selecione]</option>
													<%= getCategoria() %>
												</select>
												&nbsp;&nbsp;&nbsp;
												Data de Cadastro -
												De: <input id="dedatacadastro" type="text" class="text" name="dedatacadastro" value="<%= Request.Form("dedatacadastro") %>" size="13" readonly /> <input TYPE="button" NAME="btndata1" class="btnform" VALUE="..." Onclick="javascript:popdate('document.form1.dedatacadastro','pop1','150',document.form1.dedatacadastro.value)" /><span id="pop1" style="position:absolute;margin-left:20px;"></span>
												Até: <input id="atedatacadastro" type="text" class="text" name="atedatacadastro" value="<%= Request.Form("atedatacadastro") %>" size="13" readonly /> <input TYPE="button" NAME="btndata2" class="btnform" VALUE="..." Onclick="javascript:popdate('document.form1.atedatacadastro','pop2','150',document.form1.atedatacadastro.value)" /><span id="pop2" style="position:absolute;margin-left:20px;"></span>
											</div>
											<div align="left" style="padding:3px 3px 3px 3px;width:100%;">
											<%'if session("sql") <> "" then%>
