...
function aprovar(ID, TIPO) {
	_data();
	document.frmEditSolicitacaoColetaDomiciliarAdm.cbStatusSolColeta.value = 2;
	if (document.frmEditSolicitacaoColetaDomiciliarAdm.cbStatusSolColeta.value == 2) {
		if (confirm("Deseja realmente aprovar essa solicitação?")) {
			try {
				var oAjax = Ajax();
				oAjax.onreadystatechange = function() {}
				oAjax.open("GET","ajax/Ajax.asp?sub=aprovarsolicitacao&id="+ID+"&Tipo="+TIPO,false);
