...
<%
	...
	sub exportarParaArquivo(sqlcliente, sqlponto)
		dim i, arr, intarr
		dim j, arr2, intarr2
		dim arquivo
		dim fso
...
'		response.end
		...
		arquivo = ""
		call search(sqlcliente, arr, intarr)
		call search(sqlponto, arr2, intarr2)
		cabecalhoArq = "Data Solicitação;Número Solicitação;Cod. Cliente;Razão Social;UF Cliente;Qtd. Cartuchos;Cód. Categoria;Desc. Categoria"
		arquivoPath.writeLine(cabecalhoArq)
		if intarr > -1 then
			for i=0 to intarr
				arquivo = DateRight(arr(5,i))&";"&arr(2,i)&";"&arr(27,i)&";"&arr(30,i)&";"&arr(23,i)&";"&arr(3,i)&";"&arr(29,i)&";"&getDescCategoria(arr(29,i))
				arquivoPath.writeLine(arquivo)
			next
		end if
		if intarr2 > -1 then
			for j=0 to intarr2
				arquivo = DateRight(arr2(5,j))&";"&arr2(2,j)&";"&arr2(15,j)&";"&arr2(16,j)&";"&arr(28,j)&";"&arr(3,j)&";---;---"
				arquivoPath.writeLine(arquivo)
			next
		end if
	...
	function getSolicitacoesByCliente()
		...
		sql = "SELECT A.[idSolicitacao_coleta] " & _
				  ...
				  "where A.[isMaster] = 0 and left(A.[numero_solicitacao_coleta],1) <> 'R' "
...
Response.Write sql & "<hr>"
		...
		if intarr > -1 then
			...
			For i = (intProdsPorPag * (intPag - 1)) to intUltima
				...
				html = html & "<td "&style&">" & getPontoColetaDesc(arr(14,i)) & "</td>"
...
'		response.end
		...
		if intarr > -1 then
			for i=0 to intarr
				...
				html = html & "<tr>"
				html = html & "<td "&style&">"&DateRight(formatdatetime(arr(5,i)))&"</td>"
				html = html & "<td "&style&">"&arr(2,i)&"</td>"
				...
				html = html & "<td "&style&">---</td>"
				html = html & "<td "&style&">" & getTransportadoraDesc(arr(33,i)) & "</td>"
				html = html & "<td "&style&">" & getPontoColetaDesc(arr(14,i)) & "</td>"
				html = html & "<td "&style&">" & getStatusDesc(arr(1,i)) & "</td>"
				html = html & "</tr>"
			next
...
'Descrição: Apenas aparecer a linha quando não houver registros
		if i> 0 then
			html = html & "<tr>"
			html = html & "<td colspan=""9"" align=""center"">Nenhum registro encontrado</td>"
			html = html & "</tr>"
										...
										<fieldset style="font-size:10px;font-family:Verdana, Arial, Helvetica, sans-serif;">
											...
											<div align="left" style="padding:3px 3px 3px 3px;width:100%;">
												...
												<select name="tipo" class="select" style="width:200px;">
												  <option value="0" <% If cint(Request.Form("tipo")) = 0 Then %>selected<% End If %>>[Selecione]</option>
												  <option value="1" <% If cint(Request.Form("tipo")) = 1 Then %>selected<% End If %>>Master</option>
												  <option value="2" <% If cint(Request.Form("tipo")) = 2 Then %>selected<% End If %>>Domiciliar</option>
												  <option value="3" <% If cint(Request.Form("tipo")) = 3 Then %>selected<% End If %>>Ponto Coleta</option>
												</select>
												&nbsp;&nbsp;&nbsp;
												Status:
...
-->									
								...
								<% 
								Response.Write "#" & Request.Form("tipo") & "#<hr>"
								If cint(Request.Form("tipo")) = 1 Then %>
								...
								<%	
									if request.form("submit") = "Exportar" then
										call exportarParaArquivo(sqlExportarCliente, sqlExportarPonto)
									end if
								%>
							</table>
						</td>
