...
<%
	...
	sub exportarParaArquivo(sqlcliente)
		dim i, arr, intarr
		dim j, arr2, intarr2
		dim arquivo
		dim fso
...
'		response.end
		...
		cabecalhoArq = "Data Solicitação;Data Aprovação;Data Entrega Ponto Coleta;Data Recebimento;Número Solicitação;Cod. Cliente;Razão Social;UF Cliente;Qtd. Cartuchos;Cód. Categoria;Desc. Categoria;Ponto de coleta;Status"
		arquivoPath.writeLine(cabecalhoArq)
		if intarr > -1 then
			for i=0 to intarr
				arquivo = DateRight(arr(5,i))&";"&DateRight(arr(6,i))&";"&DateRight(arr(8,i))&";"&DateRight(arr(9,i))&";"&arr(2,i)&";"&arr(27,i)&";"&arr(30,i)&";"&arr(23,i)&";"&arr(3,i)&";"&arr(29,i)&";"&getDescCategoria(arr(29,i))&";"&getPontoColetaDesc(arr(14,i))&";"&getStatusDesc(arr(1,i))
	...
	function getSolicitacoesByCliente()
		...
		sql = "SELECT A.[idSolicitacao_coleta] " & _
				  ...
				  "LEFT JOIN Pontos_coleta AS D ON D.idPontos_coleta = B.Pontos_coleta_idPontos_coleta " & _
				  "where A.[isMaster] = 0 and left(A.[numero_solicitacao_coleta],1) = 'E' "
...
'Response.Write sql & "<hr>"
		...
		if intarr > -1 then
			...
			For i = (intProdsPorPag * (intPag - 1)) to intUltima
				...
				html = html & "<td "&style&">"&DateRight(arr(8,i))&"</td>"				
	...
	function getPontoColetaDesc(idPontoColeta)
		...
		if len(trim(idPontoColeta)) > 0 then
			sql = "SELECT [Razao_social] " & _
				  ...
				  " WHERE idPontos_coleta = " & idPontoColeta
	...
	function getWhereSQLCliente()
		...
		if existWhereCliente() then
			...
			if Request.Form("uf") <> "0" then
				if bAnd then
					sql = sql & " and D.[estado] = '" & Request.Form("uf") & "'"
				else
					sql = sql & " D.[estado] = '" & Request.Form("uf") & "'"
					bAnd = true
				end if
			end if
						...
						<td colspan="3">
							<table cellpadding="1" cellspacing="1" width="100%" id="tableRelSolPendente" style="border:1px solid #000000">
								<tr>
									...
									<th>Data Entrega Ponto Coleta</th>
