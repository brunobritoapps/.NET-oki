...
<%
	...
	sub exportarParaArquivo(sqlponto)
		...
		cabecalhoArq = "Número Solicitação;Cod. Cliente;Razão Social;UF Cliente;Qtd. Cartuchos;Transportadora; Data solicitacao; Data aprovacao; Data programada; Data recebimento; Status"
		arquivoPath.writeLine(cabecalhoArq)
		if intarr2 > -1 then
			for j=0 to intarr2
				arquivo = arr2(2,j)&";"&arr2(15,j)&";"&arr2(16,j)&";"&arr2(28,j)&";"&arr2(3,j)&";"&getTransportadoraDesc(arr2(33,j))&";"&DateRight(arr2(5,j))&";"&DateRight(arr2(6,j))&";"&DateRight(arr2(7,j))&";"&DateRight(arr2(10,j))&";"&getStatusDesc(arr2(1,j))
	...
	function getSolicitacaoByPonto()
		...
		sql = "SELECT A.[idSolicitacao_coleta] " & _
			  ...
			  "where A.[isMaster] = 1 and left(A.[numero_solicitacao_coleta],1) = 'M'"
...
'		response.end
		...
		if intarr > -1 then
			...
			For i = (intProdsPorPag * (intPag - 1)) to intUltima
				...
				html = html & "<td "&style&">"&DateRight(arr(7,i))&"</td>"
				html = html & "<td "&style&">"&DateRight(arr(10,i))&"</td>"
...
-->									
								...
								<%	
									if request.form("submit") = "Exportar" then
										call exportarParaArquivo(sqlExportarPonto)
									end if
								%>
							</table>
						</td>
