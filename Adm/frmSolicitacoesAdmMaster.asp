	...
	Sub getSolicitacoesColeta()
		...
		If trim(Request.QueryString("busca")) <> "" Then
			sSql = "SELECT " & _ 
							...
							"left join Solicitacao_coleta_has_Pontos_coleta as [e] " & _
							"on [A].[idSolicitacao_coleta] = [e].[Solicitacao_coleta_idSolicitacao_coleta] " & _
							"left join pontos_coleta as [f] " & _
							"on [e].[Pontos_coleta_idPontos_coleta] = [f].[idPontos_coleta] " & _
							"WHERE [A].[isMaster] = 1 AND [A].[numero_solicitacao_coleta] = '"&trim(Request.QueryString("busca"))&"'"
		Else
			If CInt(Request.QueryString("StatusSol")) = 0 Then
				sSql = "SELECT " & _ 
								...
								"left join pontos_coleta as [f] " & _
								"on [e].[Pontos_coleta_idPontos_coleta] = [f].[idPontos_coleta] " & _
								"WHERE [A].[isMaster] = 1 AND [A].[numero_solicitacao_coleta] NOT LIKE 'E%'"
			Else
				If CInt(Request.QueryString("StatusSol")) > 0 Then
				sSql = "SELECT " & _ 
								"[A].[idSolicitacao_coleta], " & _
								...
								"left join pontos_coleta as [f] " & _
								"on [e].[Pontos_coleta_idPontos_coleta] = [f].[idPontos_coleta] " & _
									"WHERE [A].[isMaster] = 1 AND " & _
									"[A].[Status_coleta_idStatus_coleta] = " & Request.QueryString("StatusSol") & " AND [A].[numero_solicitacao_coleta] NOT LIKE 'E%'"
				End If							
			End If
		End if	
		...
		With Response	
			If intSolColeta > -1 Then
				...
				For i = (intProdsPorPag * (intPag - 1)) to intUltima
					If i Mod 2 = 0 Then
						.Write "<tr>"
						.Write "<td width='1%' align='left' class='classColorRelPar'><img class='imgexpandeinfo' src='img/buscar.gif' alt='Verificar Solicitação de Coleta' onClick=""javascript:window.open('frmEditSolicitacaoColetaAdm.asp?iscoletadomiciliar="&arrSolColeta(8,i)&"&idsolic="&arrSolColeta(0,i)&"','','width=500,height=650,scrollbars=no,status=no,location=no,toolbar=no,menubar=no');"" ></td>"
					...
					<table cellpadding="3" cellspacing="0" width="100%">
						...
						<tr>
							<td colspan="4" id="explaintitle" align="left">
								...
								<input type="button" name="btnprocurar" value="Procurar" class="btnform" onClick="window.location.href='frmSolicitacoesAdmMaster.asp?busca=' + document.frmOperacionalAdm.busca.value + ''" />
							</td>
						</tr>
						<tr>
							<td colspan="4" id="explaintitle" align="left">
								Status : 
								<select name="cbStatusColeta" class="select" onChange="window.location.href='frmSolicitacoesAdmMaster.asp?StatusSol=' + this.value;">
							...
							<!--td id="explaintitle" align="center">
								Categoria : 
								<select name="cbTipoColeta" class="select" onChange="window.location.href='frmSolicitacoesAdmMaster.asp?TypeColect=' + this.value + '&StatusSol=' + document.frmOperacionalAdm.cbStatusColeta.value;">
									<option value="0">Todas</option>
									<%Call GetColectType()%>
								</select>	
							</td-->
