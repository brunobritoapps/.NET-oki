...
<%
	...
	function getListKardex()
		...
		if intarr > -1 then
			...
			For i = (intProdsPorPag * (intPag - 1)) to intUltima
				...
				ret = ret & "<tr>"
				ret = ret & "<td onclick=""window.location.href='frmeditkardex.asp?id="&arr(0,i)&"'"" "&style&" style=""cursor:pointer;"" title=""Administração Kardex ["&arr(7,i)&"]"">"&arr(7,i)&"</td>"
				ret = ret & "<td onclick=""window.location.href='frmeditkardex.asp?id="&arr(0,i)&"'"" "&style&" style=""cursor:pointer;"" title=""Administração Kardex ["&arr(7,i)&"]"">"&arr(1,i)&"</td>"
				if left(request.ServerVariables("LOCAL_ADDR"), 2) = "10" then
					ret = ret & "<td onclick=""window.location.href='frmeditkardex.asp?id="&arr(0,i)&"'"" "&style&" style=""cursor:pointer;"" title=""Administração Kardex ["&arr(7,i)&"]"">"&DateRight(arr(2,i))&"</td>"
				else
					ret = ret & "<td onclick=""window.location.href='frmeditkardex.asp?id="&arr(0,i)&"'"" "&style&" style=""cursor:pointer;"" title=""Administração Kardex ["&arr(7,i)&"]"">"&arr(2,i)&"</td>"
				end if
				ret = ret & "<td onclick=""window.location.href='frmeditkardex.asp?id="&arr(0,i)&"'"" "&style&" style=""cursor:pointer;"" title=""Administração Kardex ["&arr(7,i)&"]"">"&arr(3,i)&"</td>"
				ret = ret & "<td onclick=""window.location.href='frmeditkardex.asp?id="&arr(0,i)&"'"" "&style&" style=""cursor:pointer;"" title=""Administração Kardex ["&arr(7,i)&"]"">"&arr(4,i)&"</td>"
				ret = ret & "<td align=""right"" onclick=""window.location.href='frmeditkardex.asp?id="&arr(0,i)&"'"" "&style&" style=""cursor:pointer;"" title=""Administração Kardex ["&arr(7,i)&"]"">"&arr(5,i)&"</td>"
				if left(request.ServerVariables("LOCAL_ADDR"), 2) = "10" then
					ret = ret & "<td onclick=""window.location.href='frmeditkardex.asp?id="&arr(0,i)&"'"" "&style&" style=""cursor:pointer;"" title=""Administração Kardex ["&arr(7,i)&"]"">"&DateRight(arr(6,i))&"</td>"
				else
					ret = ret & "<td onclick=""window.location.href='frmeditkardex.asp?id="&arr(0,i)&"'"" "&style&" style=""cursor:pointer;"" title=""Administração Kardex ["&arr(7,i)&"]"">"&arr(6,i)&"</td>"
				end if
				ret = ret & "</tr>"
			next
			ret = ret & "<tr><td colspan=9><div id=pag>"
	...
	'====================================================================================================================
	' Bonus / Rotinas referentes ao processamento de Bônus
	'====================================================================================================================
		...
		if intarr > -1 then
			for i=0 to intarr
				...
				If codbonus = "" and i < 1 then
				 	Response.Write "<script>alert('Bônus não cadastrado para este cliente! Não será gerado bônus');</script>"
				 'response.end
				End If
				...
				if ubound(cod_produto, 2) > -1 then
...
'			response.write "boolValida: " & boolValida & "<br />"
	...
	function validaDataRecebimento(numero_solicitacao, codbonus)
		...
		sql = "select " & _
				...
				",DATEDIFF(day,         b.data_inicio_contabilizacao, a.data_recebimento) " & _
...
'								response.write "validaInsertBonusPonto(arr(0,i), arr(9,i), arr(1,i)): " & validaInsertBonusPonto(arr(0,i), arr(9,i), arr(1,i)) & "<br />"
							...
							logger = logger & "<b style=""color:#FF0000;"">Erro: Data Recebimento é menor que a Data de Inicio da Contabilização ["&arr(0,i)&"] ["&arr(9,i)&"]</b><br />"
						end if
'					end if
				else
					logger = logger & "<b style=""color:#FF0000;"">Erro: Cliente não participa do Programa de Bônus ["&arr(0,i)&"]</b><br />"
				end if
			next
		...
		if intarr > -1 then
			for i=0 to intarr
				...
				if intarr > -1 then
					...
					next
					html = html & "<tr>"
					html = html & "<td colspan=""5"" align=""right""><b>Saldo da Pontuação: </b>"&pontuacao_real&"</td>"
					html = html & "</tr>"
				else
...
'					response.write "sql atualiza kardex: " & sql & "<br />"
	...
	function getProdutoBySolicitacao(num_solicitacao)
		dim sql, arr, intarr
		sql = "select codigo_produto, qtd from kardex where numero_solicitacao_coleta = '"&num_solicitacao&"'"
		call search(sql, arr, intarr)
		if intarr > -1 then
'				qtd = trim(arr(1,i))
			...
			<tr>
				...
				<td id="conteudo">
					<table cellspacing="1" cellpadding="1" width="100%" border=0 ID="Table2">
						<tr>
							<td colspan="2" id="explaintitle" align="center">Administração do Kardex</td>
						</tr>
						<tr>
						...
						<tr id="deleteSelected">
							<td>
								<input type="submit" name="action" value="Gerar Bônus" class="btnform" />
							</td>
						</tr>
						<tr>
